...
public class TheMuonTra_Panel extends JPanel {
	 ...
	 * Create the panel.
	 */
	public TheMuonTra_Panel(ConnectSQL n,THUTHU t) {
		c=n;
		this.t=t;
	 ...
	 * */
		...
		JLabel lblNewLabel = new JLabel("QUẢN LÝ THẺ MƯỢN TRẢ");
	 ...
	 * */
		...
		btnThem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				TheMuonTra_Them them = new TheMuonTra_Them(c,t);
		...
		btnXoa.setText("Xóa");
		btnXoa.setBounds(10, 236, 192, 35);
		add(btnXoa);
		...
		btnChiTiet.setBounds(10, 281, 192, 35);
		add(btnChiTiet);
		...
		Button btnTraSach = new Button("Trả sách");
		btnTraSach.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				traSach();
			}
		});
		btnTraSach.setBounds(10, 327, 192, 35);
		add(btnTraSach);
		update();
	}
	void update() {
	...
	void xoa() {
		...
		int i = table.getSelectedRow();
		if(i>=0) {
			int a = JOptionPane.showConfirmDialog(getRootPane(), "Bạn có chắc chắn xóa?");
			if(a==0) {
				Integer ma = (int)mta.getValueAt(i, 0);
				THEMUONTRA t = new THEMUONTRA();
				t.setMaMuonTra(ma);
		...
		}else JOptionPane.showMessageDialog(getRootPane(), "Chọn thẻ cần xóa!");
	}
	...
	void traSach() {
		...
		if(i>=0) {
			...
			THEMUONTRA t = new THEMUONTRA();
			t.setMaMuonTra(ma);
			TheMuonTra_TraSach f = new TheMuonTra_TraSach(c,t);
			f.setVisible(true);
		}else JOptionPane.showMessageDialog(getRootPane(), "Chọn thẻ muốn trả!");
	}
